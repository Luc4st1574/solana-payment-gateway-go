...
func (tx *Tx) init() {
	tx.MatchResults = NewMatchResultsClient(tx.config)
	tx.Matches = NewMatchesClient(tx.config)
	tx.PaymentVerifications = NewPaymentVerificationsClient(tx.config)
...
// applies a query, for example: MatchResults.QueryXXX(), the query will be executed
